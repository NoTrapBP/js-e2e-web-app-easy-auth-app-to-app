...
const refreshToken = async function (req, _, next) {
  ...
  req.tokenMiddleware.isExpired = isTokenExpired(
    req.tokenMiddleware.decoded.exp
  );

  // If token is expired, refresh it
  if (req.tokenMiddleware.isExpired.expired) {
    const refreshUrl = `https://${req.headers.host}/.auth/refresh`;
    req.tokenMiddleware.refreshedTokenResult = await refreshTokenInMiddleware(
      refreshUrl,
      req.tokenMiddleware.token
    );
  }
  return next();
};
...
export const create = async () => {
  ...
  app.get('/get-profile', async (req, res) => {
    try {
      ...
      let remoteUrl = process.env.BACKEND_URL + '/httptrigger';
      if (!remoteUrl) {
        return res.render(`${__dirname}/views/profile`, {
          error: 'Client: No remote URL found',
        });
      }
      ...
      const authEnabled =
        process.env.APPSETTING_WEBSITE_AUTH_ENABLED === 'true' ? true : false;
      ...
      // Get remote profile
      const response = await getRemoteProfile(
        remoteUrl,
        accessToken,
        authEnabled
      );
